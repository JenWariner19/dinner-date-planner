...
//This 
  ...
  drinkArrayID = JSON.parse(localStorage.getItem("drinkID"));
   if (mealArray === null && mealArrayID === null && drinkArray === null && drinkArrayID === null){
    mealArray = [];
    mealArrayID = [];
    drinkArray= [];
    drinkArrayID = [];
...
//   data.push(inputValue);
  ...
  for (var i = 0; i < mealArray.length; i++) {
    ...
    savedHTML += '<li mealID="' + mealArrayID[i] + '" drinkID="' + drinkArrayID[i] + '">' + mealArray[i] + ' and ' + drinkArray[i] + '</li>';
